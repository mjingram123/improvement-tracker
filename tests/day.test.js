...
test('defaultMorning: all false when no legacy stretched value', () => {
  assert.deepEqual(D.defaultMorning(undefined), { up: false, pushups: false, stretched: false, shower: false, supplements: false });
});
test('defaultMorning: seeds stretched from the legacy flag', () => {
  assert.deepEqual(D.defaultMorning(true), { up: false, pushups: false, stretched: true, shower: false, supplements: false });
});
test('getMorning: builds defaults when d.morning is missing, carrying over legacy stretched', () => {
  const d = { stretched: true };
  assert.deepEqual(D.getMorning(d), { up: false, pushups: false, stretched: true, shower: false, supplements: false });
});
test('getMorning: returns d.morning merged over defaults when present', () => {
  const d = { stretched: false, morning: { up: true, pushups: true } };
  assert.deepEqual(D.getMorning(d), { up: true, pushups: true, stretched: false, shower: false, supplements: false });
});
...
test('morningAllChecked: false until every one of the four is checked', () => {
  assert.equal(D.morningAllChecked({ up: true, pushups: true, stretched: true, shower: false, supplements: false }), false);
  assert.equal(D.morningAllChecked({ up: true, pushups: true, stretched: true, shower: true, supplements: true }), true);
});

// ---------- morningCardDone ----------
test('morningCardDone: done when all four checked and hangover row not active', () => {
  const morning = { up: true, pushups: true, stretched: true, shower: true, supplements: true };
...
test('morningCardDone: not done if any of the four is unchecked, even with no hangover row', () => {
  const morning = { up: true, pushups: true, stretched: true, shower: false, supplements: false };
...
// ---------- MORNING_ITEMS ----------
test('MORNING_ITEMS: five items, matching contract keys', () => {
  assert.deepEqual(D.MORNING_ITEMS.map((i) => i.key), ['up', 'pushups', 'stretched', 'shower', 'supplements']);
});
